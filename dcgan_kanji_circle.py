...
def load_kanji_data():
    imgs = glob.glob("./fontImg/*.png")
    print len(imgs)
    value = np.zeros((len(imgs),1,28,28))
...
def train(args):
    ...
    with nn.parameter_scope("dis"):
		nn.load_parameters("/home/mizuochi/programing/font/tmp.monitor.dcgan.vectorizer/vectorizer_param_220000.h5")
    ...
    for i in range(args.batch_size):
        Image.fromarray(np.uint8((gen.d[i][0]+1)*255/2.0)).save("./testCircle/"+str(i)+"_.png")
        Image.fromarray(np.uint8((x.d[i][0]+1)*255/2.0)).save("./testCircle/"+str(i)+".png")
